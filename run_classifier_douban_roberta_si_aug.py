...
from Utils.DataLoader import DoubanDataset
...
def main():
    ...
    parser.add_argument("--dialog_augmentation_path",
                        default='/hdd/lujunyu/dataset/multi_turn_corpus/douban/train_augment_3.txt', ## train_augment_3.txt
                        type=str,
    ...
    bert_config = BertConfig.from_pretrained(args.init_model_path+'config.json', num_labels=2)
    ...
    tokenizer = BertTokenizer.from_pretrained(args.init_model_path+'vocab.txt', do_lower_case=args.do_lower_case)
    if args.dialog_augmentation_path:
        train_dataset = DoubanDataset(
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size,
                                                sampler=RandomSampler(train_dataset), num_workers=9)
    eval_dataloader = torch.utils.data.DataLoader(eval_dataset, batch_size=args.eval_batch_size,
                                                sampler=SequentialSampler(eval_dataset), num_workers=9)

    model = BertForSequenceClassification.from_pretrained('/hdd/lujunyu/model/chatbert/douban_roberta_si_aug_beifen/model.pt', config=bert_config)
    model.to(device)
