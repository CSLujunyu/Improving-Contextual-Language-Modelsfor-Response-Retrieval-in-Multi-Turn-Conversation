...
def main():
    ...
    parser.add_argument("--output_dir",
                        default='/hdd/lujunyu/model/wwm_data_aug/',
                        type=str,
                        required=False,
    ...
    tokenizer = BertTokenizer.from_pretrained(os.path.join(args.init_checkpoint, 'vocab.txt'), do_lower_case=args.do_lower_case)
    ...
    config = BertConfig.from_pretrained(args.init_checkpoint, num_labels=2)
    model = BertForSequenceClassification.from_pretrained(args.init_checkpoint, args, config=config)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.save_checkpoints_steps == 0:
                    ...
                    if best_acc < eval_accuracy:
                        ...
                        visualize_bad_cases(
                            logits=logits_all,
                            input_file_path=os.path.join(args.data_dir, 'dev.txt'),
                            output_file_path=os.path.join(args.output_dir, 'dev_bad_cases.txt')
                        )
