...
from Utils.DataLoader import UbuntuDatasetForRobertaSP
from pytorch_transformers import AdamW, WarmupLinearSchedule, RobertaConfig, RobertaForSequenceClassification
from Utils.transformers.tokenization_roberta import RobertaTokenizer
...
def main():
    ...
    parser.add_argument("--output_dir",
                        default='/hdd/lujunyu/model/chatbert/ubuntu_roberta_sp/',
                        type=str,
                        required=False,
    ...
    if args.data_augmentation:
        train_dataset = UbuntuDatasetForRobertaSP(
    ...
    eval_dataset = UbuntuDatasetForRobertaSP(
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, _ = model(input_ids, token_type_ids=segment_ids, labels=label_ids)
                if n_gpu > 1:
                ...
                if (step + 1) % args.save_checkpoints_steps == 0:
                    ...
                    logits_all = []
                    for input_ids, segment_ids, label_ids in eval_dataloader:
                        input_ids = input_ids.to(device)
                        segment_ids = segment_ids.to(device)
                        label_ids = label_ids.to(device)

                        with torch.no_grad():
                            tmp_eval_loss, logits = model(input_ids, token_type_ids=segment_ids, labels=label_ids)
